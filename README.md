...
2. Создать файлы `.env` , `.env-docker` на основе `.env.example` и `.env-docker-example`, заполнить ключи API, настройки БД и Redis.
...
- подсчет токенов, время ответа в логах.

## Eval

- Eval для проверки качества ответов в логах.
